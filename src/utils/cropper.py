...
class Cropper(object):
    def __init__(self, **kwargs) -> None:
        ...
        self.halfbody = kwargs.get("halfbody", False)
    ...
    def crop_driving_video(self, driving_rgb_lst, **kwargs):
        ...
        direction = kwargs.get("direction", "large-small")
        face_detected = False
        for idx, frame_rgb in tqdm(enumerate(driving_rgb_lst)):
            if idx == 0 or trajectory.start == -1:
                src_face = self.face_analysis_wrapper.get(
                    contiguous(frame_rgb[..., ::-1]),
                    flag_do_landmark_2d_106=True,
        ...
        if self.halfbody:
            dx = global_bbox[2] - global_bbox[0]
            dy = global_bbox[3] - global_bbox[1]
            global_bbox[0] -= dx / 2
            global_bbox[2] += dx / 2
            global_bbox[3] += dy
            print(global_bbox)
        ...
        for idx, (frame_rgb, lmk) in enumerate(
            ...
            ret_dct = crop_image_by_bbox( # HACK: TODO: check this
                frame_rgb,
                global_bbox,
                lmk=lmk,
            ...
            # cv2.imwrite(f"./frames_to_check/frame_crop_check_{kwargs.get('number', 1)}.png", ret_dct["img_crop"][:,:,::-1])
            # import sys
            # sys.exit()
    ...
    def get_global_bbox(self, driving_rgb_lst: Iterable[np.ndarray], **kwargs):
        trajectory = Trajectory()
        direction = kwargs.get("direction", "large-small")
        for idx, frame_rgb in enumerate(driving_rgb_lst):
            if trajectory.start == -1:
                src_face = self.face_analysis_wrapper.get(
                    contiguous(frame_rgb[..., ::-1]),
                    flag_do_landmark_2d_106=True,
                    direction=direction,
    ...
    def crop_driving_video_v2(self, driving_rgb_lst, **kwargs):
        """This version is for avoiding OOM issue"""
        trajectory = Trajectory()
        direction = kwargs.get("direction", "large-small")
        for idx, frame_rgb in tqdm(
            enumerate(driving_rgb_lst),
            total=len(driving_rgb_lst),
            desc="Detecting landmarks and parsing bbox",
        ):
            if trajectory.start == -1:
                src_face = self.face_analysis_wrapper.get(
                    contiguous(frame_rgb[..., ::-1]),
        ...
        for idx, frame_rgb in tqdm(
            enumerate(driving_rgb_lst),
            total=len(driving_rgb_lst),
            desc="Cropping driving video",
        ):
            if idx < trajectory.start:
                continue
            elif idx > trajectory.end:
                break
            ret_dct = crop_image_by_bbox_(
                frame_rgb,
                global_bbox,
                lmk=trajectory.lmk_lst[idx - trajectory.start],
                dsize=kwargs.get("dsize", 512),
                flag_rot=False,
                borderValue=(0, 0, 0),
            )
            ret_dct["crop_box"] = global_bbox
            ret_dct["start_frame_idx"] = trajectory.start
            yield ret_dct
